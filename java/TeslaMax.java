...
public class TeslaMax {
	...
	private static String ironIIWedge1QTag;
	...
	private static GeomFeature ironIIWedge1QFeature;
	...
	private static GeomFeature buildIronBlock(String magnetRegion) {
		...
		if (magnetRegion.equals("II")) {
			...
			expression = new String[] { "R_g", "R_s", "phi_S_IV", "90[deg]" };
		} else if (magnetRegion.equals("IIw1Q")) {
	...
	private static void createSelections() {
		...
		SelectionFeature environmentBoundarySelection = model.selection()
				.create(ENVIRONMENT_HORIZONTAL_BOUNDARY_SELECTION_TAG, "Union");
		...
		if (ironIIWedge1QTag != null) {
			model.selection(IRON_SELECTION_TAG).add(getDomainEntities(ironIIWedge1QTag));
		}
		...
		model.selection(CIRCUIT_REGIONS_SELECTION_TAG).add(model.selection(MAGNETS_SELECTION_TAG).entities(2));
		model.selection(CIRCUIT_REGIONS_SELECTION_TAG).add(model.selection(IRON_SELECTION_TAG).entities(2));

		// to apply the zero magnetic scalar potential to the line x=0,
		// we create a box that encompasses all relevant edges
		model.selection().create(ZERO_POTENTIAL_SELECTION_TAG, "Box");

		model.selection(ZERO_POTENTIAL_SELECTION_TAG).set("entitydim", 1);
		model.selection(ZERO_POTENTIAL_SELECTION_TAG).set("condition", "inside");
		model.selection(ZERO_POTENTIAL_SELECTION_TAG).set("xmin", -r);
		model.selection(ZERO_POTENTIAL_SELECTION_TAG).set("xmax", 0);
		model.selection(ZERO_POTENTIAL_SELECTION_TAG).set("ymin", 0);
		model.selection(ZERO_POTENTIAL_SELECTION_TAG).set("ymax", R_e + r);
		model.selection(ZERO_POTENTIAL_SELECTION_TAG).label("x = 0 line");
	...
	private static void createProbes() {
		...
		demagnetizedAreaProbe.set("expr",
				"if((-(mfnc.Hx * mfnc.Brx + mfnc.Hy * mfnc.Bry) / mfnc.normBr - Hc_j)>0, 4, 0)");
	...
	private static void configureAirMaterial(Material mat) {
		...
		mat.propertyGroup("def").func("eta").set("pieces", new String[][] { { "200.0", "1600.0",
		...
		mat.propertyGroup("def").func("Cp").set("pieces", new String[][] { { "200.0", "1600.0",
				"1047.63657-0.372589265*T^1+9.45304214E-4*T^2-6.02409443E-7*T^3+1.2858961E-10*T^4" } });
		...
		mat.propertyGroup("def").func("rho").set("plotargs", new String[][] { { "pA", "0", "1" }, { "T", "0", "1" } });
		mat.propertyGroup("def").func("rho").set("argders",
				new String[][] { { "pA", "d(pA*0.02897/8.314/T,pA)" }, { "T", "d(pA*0.02897/8.314/T,T)" } });
		mat.propertyGroup("def").func("k").set("pieces", new String[][] { { "200.0", "1600.0",
		...
		mat.propertyGroup("def").func("cs").set("argders", new String[][] { { "T", "d(sqrt(1.4*287*T),T)" } });
		mat.propertyGroup("def").set("relpermeability", new String[] { "1", "0", "0", "0", "1", "0", "0", "0", "1" });
		mat.propertyGroup("def").set("relpermittivity", new String[] { "1", "0", "0", "0", "1", "0", "0", "0", "1" });
		...
		mat.propertyGroup("def").set("thermalconductivity", new String[] { "k(T[1/K])[W/(m*K)]", "0", "0", "0",
		...
		mat.propertyGroup("RefractiveIndex").set("n", new String[] { "1", "0", "0", "0", "1", "0", "0", "0", "1" });
		mat.propertyGroup("RefractiveIndex").set("ki", new String[] { "0", "0", "0", "0", "0", "0", "0", "0", "0" });
	}
	...
	private static void configureIronMaterialLinear(Material mat) {
		...
		mat.propertyGroup("def").set("relpermeability",
				new String[] { "mu_r_iron", "0", "0", "0", "mu_r_iron", "0", "0", "0", "mu_r_iron" });
	...
	private static void configureIronMaterialNonLinear(Material mat) {
		...
		mat.propertyGroup("def").set("electricconductivity",
				new String[] { "1.12e7[S/m]", "0", "0", "0", "1.12e7[S/m]", "0", "0", "0", "1.12e7[S/m]" });
		mat.propertyGroup("def").set("relpermittivity", new String[] { "1", "0", "0", "0", "1", "0", "0", "0", "1" });
		mat.propertyGroup("BHCurve").func("BH").set("extrap", "linear");
		mat.propertyGroup("BHCurve").func("BH").set("table",
				new String[][] { { "0", "0" }, { "663.146", "1" }, { "1067.5", "1.1" }, { "1705.23", "1.2" },
						{ "2463.11", "1.3" }, { "3841.67", "1.4" }, { "5425.74", "1.5" }, { "7957.75", "1.6" },
						{ "12298.3", "1.7" }, { "20462.8", "1.8" }, { "32169.6", "1.9" }, { "61213.4", "2.0" },
						{ "111408", "2.1" }, { "175070", "2.2" }, { "261469", "2.3" }, { "318310", "2.4" } });
		...
		mat.propertyGroup("HBCurve").func("HB").set("table",
				new String[][] { { "0", "0" }, { "1", "663.146" }, { "1.1", "1067.5" }, { "1.2", "1705.23" },
						{ "1.3", "2463.11" }, { "1.4", "3841.67" }, { "1.5", "5425.74" }, { "1.6", "7957.75" },
						{ "1.7", "12298.3" }, { "1.8", "20462.8" }, { "1.9", "32169.6" }, { "2.0", "61213.4" },
						{ "2.1", "111408" }, { "2.2", "175070" }, { "2.3", "261469" }, { "2.4", "318310" } });
		...
		mat.propertyGroup("EffectiveBHCurve").func("BHeff").set("table",
				new String[][] { { "0", "0" }, { "663.146", "1" }, { "1067.5", "1.4943906486860214" },
						{ "1705.23", "1.941630073817125" }, { "2463.11", "2.257619494050335" },
						{ "3841.67", "2.729755059668001" }, { "5425.74", "2.8756651489647296" },
						{ "7957.75", "3.149029234016385" }, { "12298.3", "3.4529372126833744" },
						{ "20462.8", "3.7845615956017395" }, { "32169.6", "4.060195910283011" },
						{ "61213.4", "4.421777266072753" }, { "111408", "4.721954005107204" },
						{ "175070", "4.941198649690261" }, { "261469", "5.1446599438425515" },
		...
		mat.propertyGroup("EffectiveHBCurve").func("HBeff").set("table",
				new String[][] { { "0", "0" }, { "1", "663.146" }, { "1.4943906486860214", "1067.5" },
						{ "1.941630073817125", "1705.23" }, { "2.257619494050335", "2463.11" },
						{ "2.729755059668001", "3841.67" }, { "2.8756651489647296", "5425.74" },
						{ "3.149029234016385", "7957.75" }, { "3.4529372126833744", "12298.3" },
						{ "3.7845615956017395", "20462.8" }, { "4.060195910283011", "32169.6" },
						{ "4.421777266072753", "61213.4" }, { "4.721954005107204", "111408" },
						{ "4.941198649690261", "175070" }, { "5.1446599438425515", "261469" },
	...
	private static double calculateRemanenceAngle(int index, String magnet, String quadrant) {
		...
		// it can be shown that, to maintain the two-pole symmetry, the y components of
		// the 1st and 2nd
	...
	private static void configureMagnetFluxConservation(int index, String magnet, String quadrant) {
		...
		feature.set("mur", new String[][] { { mu_r_expr }, { "0" }, { "0" }, { "0" }, { mu_r_expr }, { "0" }, { "0" },
				{ "0" }, { mu_r_expr } });
	...
	private static void configureMesh() {
		...
		meshSizeConfiguration.set("hnarrow", "n_narrow");
		meshSizeConfiguration.set("hmax", "l_element_max");
		meshSizeConfiguration.set("hmin", "l_element_min");
	...
	private static void configureResults() {
		...
		magnetsEnergyPlot.set("expr",
				"abs((mfnc.Bx*mfnc.Brx+mfnc.By*mfnc.Bry)/mfnc.normBr)*abs((mfnc.Hx*mfnc.Brx+mfnc.Hy*mfnc.Bry)/mfnc.normBr)");
		magnetsEnergyPlot.set("rangedatamax", "409");
		magnetsEnergyPlot.set("descr",
				"abs((mfnc.Bx*mfnc.Brx+mfnc.By*mfnc.Bry)/mfnc.normBr)*abs((mfnc.Hx*mfnc.Brx+mfnc.Hy*mfnc.Bry)/mfnc.normBr)");
		...
		magnetII1QDataExport.set("expr",
				new String[] { "mfnc.Bx", "mfnc.By", "mfnc.Hx", "mfnc.Hy", "mfnc.Brx", "mfnc.Bry" });
